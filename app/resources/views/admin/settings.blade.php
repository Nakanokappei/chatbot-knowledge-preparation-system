...
@section('body')
    <div class="page-content">
        <div class="page-container">
            <h1 style="font-size: 20px; font-weight: 600; margin-bottom: 4px;">{{ __('ui.system_models') }}</h1>
            <p style="color: #5f6368; font-size: 13px; margin-bottom: 24px;">{{ __('ui.system_models_desc') }}
                <a href="https://aws.amazon.com/bedrock/pricing/" target="_blank" rel="noopener" style="color: #7C3AED; text-decoration: none;">AWS Bedrock Pricing ↗</a>
            </p>
                        ...
                        <tbody>
                            ...
                            <tr @if(!$model->is_active) style="opacity: 0.5;" @endif>
                                ...
                                <td style="white-space: nowrap; font-size: 12px;">
                                    <form method="POST" action="{{ route('admin.settings.update', $model) }}" style="display: inline; margin: 0;">
                                        @csrf @method('PUT')
...
@section('scripts')
        ...
        function updateEmbDisplayName(select) {
            const option = select.options[select.selectedIndex];
            document.getElementById('emb_display_name').value = option.getAttribute('data-display') || '';
        }
        function savePricing(input) {
            ...
            }).catch(() => { input.style.borderColor = '#ff3b30'; });
        }
@endsection
